...
async function handleApiRequest(req, res) {
  ...
  try {
    ...
    // Get parlors by state using query param
    if (req.url.startsWith('/api/parlors') && params.has('state')) {
    ...
    if (req.url.startsWith('/api/parlors') && !req.url.includes('/') && !params.has('state')) {
      const limit = parseInt(params.get('limit') || '100');
      const offset = parseInt(params.get('offset') || '0');
